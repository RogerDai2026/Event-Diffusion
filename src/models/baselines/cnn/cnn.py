...
class SEVIRUNet(nn.Module):
    ...
    def __init__(self, in_channels: int, out_channels: int, start_filters: int = 64):
        ...
        # Encoder Blocks
        self.encoder0 = EncoderBlock(self.in_channels, start_filters)
        ...
        self.decoder0 = DecoderBlock(start_filters * 2, in_channels, start_filters)
        # Final Convolution to map to output channels
        self.final_conv = nn.Conv2d(start_filters, self.out_channels, kernel_size=1)
        return
...
class DecoderBlock(nn.Module):
    """
    NOTE: implementation is modified to allow for different input sizes
    """
