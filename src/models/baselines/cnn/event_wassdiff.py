...
class EventWassDiffLitModule(LightningModule):
    ...
    def test_step(self, batch: Dict[str, torch.Tensor], batch_idx: int) -> Dict[str, torch.Tensor]:
        # Optional: skip if flagged by a callback
        # Fill any missing keys, then build condition & ground‑truth
        batch = self._fill_missing_keys(batch)
        condition, gt = self._generate_condition(batch)
        ...
        # Return exactly the keys your on_test_batch_end hooks look for:
        return {
            "batch_dict": batch,
