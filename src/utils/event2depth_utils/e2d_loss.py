...
def ordinal_depth_loss(prediction: torch.Tensor, target: torch.Tensor, voxel: torch.Tensor, percent=1.0,
                       method="classical"):
    def cost(z_i, z_j, r):
        if r == 1:
    ...
    if method == "classical":
        ...
        ordinal = torch.tensor(
            [1 if (a > b) else -1 if (a < b) else 0 for a, b in zip(mtarget, mtarget.flip(dims=[0]))], dtype=torch.int8)
        ...
        ordinal = torch.tensor([1 if (a > b) else -1 if (a < b) else 0 for a, b in zip(mtarget, mprediction)],
                               dtype=torch.int8)
...
# put this into src/utils/event2depth_utils/e2d_loss.py

import torch
import torch.nn as nn

# we reuse your existing helpers:
# - scale_invariant_loss(pred, gt, weight=..., n_lambda=...)
# - MultiScaleGradient(start_scale=1, num_scales=...)

class E2DPaperLoss(nn.Module):
    """
    E2Depth paper loss:
      L = w_si * L_si( R ) + w_grad * L_grad( R )   with R = pred_hat - gt_hat
    pred_hat, gt_hat are BOTH normalized log-depth in [0,1].
    """
    def __init__(self, w_si: float = 1.0, w_grad: float = 0.5, si_lambda: float = 1.0, grad_scales: int = 4):
        super().__init__()
        self.w_si = float(w_si)
        self.w_grad = float(w_grad)
        self.si_lambda = float(si_lambda)
        self.ms_grad = MultiScaleGradient(start_scale=1, num_scales=int(grad_scales))

    def forward(self, pred_hat: torch.Tensor, gt_hat: torch.Tensor, voxel: torch.Tensor = None, valid_mask: torch.Tensor = None):
        # mask: ignore invalid GT (NaNs, infs)
        if valid_mask is None:
            valid_mask = torch.isfinite(gt_hat)
        else:
            valid_mask = valid_mask & torch.isfinite(gt_hat)
        ...
        return total, {"si": si.detach(), "grad": grad.detach()}
