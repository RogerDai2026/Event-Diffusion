...
def delta_125(y_input, y_target, eps=1e-12):
    valid = (~np.isnan(y_input)) & (~np.isnan(y_target)) & (y_input > 0) & (y_target > 0)
    yp = np.clip(y_input[valid], eps, None)
    yt = np.clip(y_target[valid], eps, None)
    ratio = np.maximum(yp / yt, yt / yp)
    return (ratio < 1.25).mean()

def delta_125_2(y_input, y_target, eps=1e-12):
    valid = (~np.isnan(y_input)) & (~np.isnan(y_target)) & (y_input > 0) & (y_target > 0)
    yp = np.clip(y_input[valid], eps, None)
    yt = np.clip(y_target[valid], eps, None)
    ratio = np.maximum(yp / yt, yt / yp)
    return (ratio < 1.25**2).mean()

def delta_125_3(y_input, y_target, eps=1e-12):
    valid = (~np.isnan(y_input)) & (~np.isnan(y_target)) & (y_input > 0) & (y_target > 0)
    yp = np.clip(y_input[valid], eps, None)
    yt = np.clip(y_target[valid], eps, None)
    ratio = np.maximum(yp / yt, yt / yp)
    return (ratio < 1.25**3).mean()

def rmse_log(y_input, y_target, eps=1e-12, multiply_by_100=False):
    """
    RMSE(log): sqrt( mean( (log(y_input) - log(y_target))^2 ) )
    """
    d = np.log(np.clip(y_input, eps, None)) - np.log(np.clip(y_target, eps, None))
    v = ~np.isnan(d)
    rmse = np.sqrt((d[v] ** 2).mean())
    return rmse * (100.0 if multiply_by_100 else 1.0)

def scale_invariant_log(y_input, y_target, eps=1e-12, multiply_by_100=False):
    """
    SIlog: sqrt( mean(d^2) - (mean(d))^2 ), where d = log(y_input) - log(y_target)
    """
    d = np.log(np.clip(y_input, eps, None)) - np.log(np.clip(y_target, eps, None))
    v = ~np.isnan(d)
    si = np.sqrt((d[v] ** 2).mean() - (d[v].mean()) ** 2)
    return si * (100.0 if multiply_by_100 else 1.0)

def abs_err_upto(y_input, y_target, cutoff_m: float):
    """Mean |pred-gt| over pixels with gt ≤ cutoff_m. Returns NaN if none."""
    abs_diff = np.abs(y_target - y_input)
    valid = (~np.isnan(abs_diff)) & (y_target <= float(cutoff_m))
    if not np.any(valid):
        return np.nan
    return abs_diff[valid].mean()

def abs_err_10m(y_input, y_target):
    return abs_err_upto(y_input, y_target, 10.0)

def abs_err_20m(y_input, y_target):
    return abs_err_upto(y_input, y_target, 20.0)

def abs_err_30m(y_input, y_target):
    return abs_err_upto(y_input, y_target, 30.0)
