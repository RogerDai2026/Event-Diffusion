...
def regression_step_only(
        net: torch.nn.Module,
        img_lr: torch.Tensor,
        latents_shape: torch.Size,
        lead_time_label: torch.Tensor = None) -> torch.Tensor:
    """
    ...
    with torch.inference_mode():
        if lead_time_label is not None:
            x = net(x_hat, img_lr, t_hat, lead_time_label=lead_time_label)
        else:
            x = net(x_hat, img_lr, t_hat)
    return x
