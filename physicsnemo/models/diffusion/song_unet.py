...
class SongUNetPosEmbd(SongUNet):
    ...
    def forward(
        ...
        #     pe = self.sinusoidal_embedding_from_index(x, global_index)
    ...
    def sinusoidal_embedding_from_index(self, x, global_index):
        """
        global_index: [B,2,Hc,Wc], dims 0=y,1=x give absolute pixel coords in full frame.
        Returns [B, N_pe, Hc, Wc].
        """
        if global_index is None:
            return (
                self.pos_embd.to(x.dtype)
        ...
        B, _, Hc, Wc = global_index.shape
        # Absolute coords:
        ys = global_index[:, 0].float()  # [B, Hc, Wc]
        xs = global_index[:, 1].float()
        ...
        # Build frequency bands (same as your gridtype='sinusoidal'):
        # here assume N_grid_channels = 4 → two bands: sin,cos for y and x
        # for more channels, just extend freq_bands = 2**linspace…
        freq_bands = torch.tensor([1.0], device=θy.device)  # one frequency band
        # if you wanted more: freq_bands = 2.0 ** torch.linspace(0, num_freq, steps=num_freq, device=θy.device)

        # Expand dims for broadcasting:
        θy = θy.unsqueeze(1) * freq_bands.view(-1, 1, 1, 1)  # [B, F, Hc, Wc]
        θx = θx.unsqueeze(1) * freq_bands.view(-1, 1, 1, 1)
        ...
        pe = torch.cat([sin_y, cos_y, sin_x, cos_x], dim=1)  # [B, 4*F, Hc, Wc]
        return pe
